...
HTMLtree = etree.parse("/item-list.html", parser)
...
if "delete" in params and "id" in params:
    item_id = params.getvalue("id")
    if params.getvalue("delete") == "true":        
        # delete the db entry for this isbn
        query = """
        delete from raw_bookmetadata
        where isbn = '%s' """ % (item_id)
        dB.ChangeQuery(query)        
        dB.Commit()

        # delete the db entry for this bookmetadata_id
        query = """
        delete from raw_bookprice
        where bookmetadata_id = '%s' """ % (item_id)
        dB.ChangeQuery(query)        
        dB.Commit()

if "publisher" in params:
    publisher_id = params.getvalue("publisher")

#get the count of promos for paging purposes
form = cgi.FieldStorage()
if "total-items" in form:
    totalitems = form["total-items"].value

#if totalitems is not set, retrieve number of publishers from database
if not 'totalitems' in locals():
    query = "select count(*) from raw_bookmetadata"
...
item_count[0].text = str(totalitems)+" Total Items"
...
if 1 < int(PAGE) < num_pages:
  for btn in prevbtn:
    btn.set("href","itemlist.py?display="+DISPLAY+"&page="+str(int(PAGE) - 1))
  for btn in nextbtn:
    btn.set("href","itemlist.py?display="+DISPLAY+"&page="+str(int(PAGE) + 1))
elif int(PAGE) == 1:
  for btn in nextbtn:
